...
export function Page1Demo({ onOpenPreview }: Props) {
  ...
  const previewMap = useMemo(() => {
    const inputSrc = uploadedPreviewUrl || selectedSample?.inputImage || "";
  ...
  }, [selectedSample, uploadedPreviewUrl]);
  ...
  const handleSampleChange = (sampleId: string | null) => {
    if (!sampleId) {
      ...
      setInferenceReady(Boolean(uploadedPreviewUrl));
      return;
    }
          ...
          <label className="upload-box">
            <input
              ...
              onChange={(event) => {
                ...
                setInferenceReady(Boolean(file) || Boolean(selectedSample));
                setStatusText(file ? "本地预览已更新" : "推理完成");
              }}
            />
        ...
        {isEmptyState ? (
          ...
          <>
            ...
            <div className="metric-bar">
              ...
              <MetricRow label="推理耗时" value={selectedSample && !isInferring ? selectedSample.inferenceTime : "--"} />
